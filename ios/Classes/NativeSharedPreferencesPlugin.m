...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar> *)registrar {
  ...
  [channel setMethodCallHandler:^(FlutterMethodCall *call, FlutterResult result) {
    ...
    if ([method isEqualToString:@"getAll"]) {
      ...
      [[[NSUserDefaults alloc] initWithSuiteName:@"group.mattcianna.Filosofando"] setBool:value.boolValue forKey:key];
      result(@YES);
      ...
      [[[NSUserDefaults alloc] initWithSuiteName:@"group.mattcianna.Filosofando"] setValue:value forKey:key];
      result(@YES);
    } else if ([method isEqualToString:@"setDouble"]) {
      NSString *key = arguments[@"key"];
      NSNumber *value = arguments[@"value"];
      [[[NSUserDefaults alloc] initWithSuiteName:@"group.mattcianna.Filosofando"] setDouble:value.doubleValue forKey:key];
      result(@YES);
      ...
      [[[NSUserDefaults alloc] initWithSuiteName:@"group.mattcianna.Filosofando"] removeObjectForKey:arguments[@"key"]];
      result(@YES);
    } else if ([method isEqualToString:@"clear"]) {
      NSUserDefaults *defaults = [[NSUserDefaults alloc] initWithSuiteName:@"group.mattcianna.Filosofando"];
